...
public class DebeziumConfigBuilder {
     ...
     * 获取数据源debezium连接配置
     *
     * @param connectorClazz 数据库类型class
     * @return debezium连接配置
     * @throws Exception 可能抛出的异常
     */
    public static Configuration build(DebeziumProperties properties, Class<?> connectorClazz) throws Exception {
        checkFile(properties.getStorageFile());
        Configuration.Builder builder = Configuration.create()
                .with("name", connectorClazz.getSimpleName())
                //连接器运行模式而不是数据的快照。当您不需要主题包含数据的一致快照，而只需要主题包含自连接器启动以来的更改时，此设置非常有用
                .with("snapshot.mode", "Schema_only")
                //监控的数据库类型
                .with("connector.class", connectorClazz)
